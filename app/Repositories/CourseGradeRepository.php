...
class CourseGradeRepository
{
    public function getCourseGradesEvolution(int $studentId)
    {
        ...
        $materiasConNotas = DB::table('materias_planes as mp')
            ...
            ->select(
                ...
                'nc.Cursada as Estado_Cursada',
                'nc.Promocion',
                'nc.Calificacion_Cursada',
                'nc.Fecha_Cursada',
                'nc.Calificacion as Calificacion_Final',
                'nc.Fecha as Fecha_Final',
                'nc.Final as Estado_Final',
                'nc.Causa',
                'nc.Observaciones',
                'nc.ID as Tiene_Nota'
            )
    ...
    {
        if (!$mat->Tiene_Nota || $mat->Estado_Cursada != 1) {
            return ''; 
        }

        if ($mat->Estado_Final == 'SI') {
            $fecha = $mat->Fecha_Final ? Carbon::parse($mat->Fecha_Final)->format('d/m/Y') : 'S/F';
            return "{$mat->Calificacion_Final} ({$fecha})";
        }
